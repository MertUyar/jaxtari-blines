...
def evaluate(
    ...
    key, reset_key, network_key, r_key, step_fn_key = jax.random.split(key, 5)
    ...
    actor_params = actor.init(network_key, dummy_obs)
    ...
    @jax.jit
    def get_action(actor_params: flax.core.FrozenDict, next_obs: jnp.ndarray):
    ...
    def step_fn(carry, _):
        ...
        actions = get_action(actor_params, next_obs)
    ...
    reset_keys = jax.random.split(r_key, eval_episodes)
